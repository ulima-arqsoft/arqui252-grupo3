...
## Otros Tipos de Comunicación Bidireccional en Tiempo Real

Aunque WebSockets es la táctica principal empleada en este proyecto, existen otros mecanismos de comunicación en tiempo real que pueden utilizarse dependiendo del contexto, la arquitectura y los requisitos no funcionales del sistema. A continuación se mencionan brevemente:

### 1. Server-Sent Events (SSE)

Canal unidireccional donde **solo el servidor envía datos al cliente**.

- **Casos de uso:** notificaciones continuas, dashboards en tiempo real, actualizaciones de sensores, feeds en streaming.
- **Por qué elegirlo:** es más simple que WebSockets cuando no se necesita enviar datos del cliente al servidor.

---

### 2. Long Polling (HTTP Polling)

El cliente realiza una solicitud HTTP que el servidor mantiene abierta hasta tener datos para enviar.

- **Casos de uso:** compatibilidad con navegadores antiguos o entornos donde no es posible usar WebSockets.
- **Por qué elegirlo:** funciona incluso en infraestructuras que bloquean WebSockets, aunque con mayor latencia y consumo de recursos.

---

### 3. WebRTC (Peer-to-Peer)

Tecnología orientada a comunicación directa entre navegadores.

- **Casos de uso:** videollamadas, voz en tiempo real, transferencia de archivos P2P.
- **Por qué elegirlo:** ideal cuando se requiere comunicación multimedia con muy baja latencia sin pasar por el servidor.

---

### 4. MQTT (Message Queue Telemetry Transport)

Protocolo ligero orientado a IoT y comunicación máquina a máquina.

- **Casos de uso:** sensores, dispositivos IoT, sistemas distribuidos de bajo consumo.
- **Por qué elegirlo:** extremadamente eficiente y optimizado para redes inestables o dispositivos con recursos limitados.

---

### 5. gRPC con Streaming

Framework RPC que permite streams bidireccionales utilizando HTTP/2.

- **Casos de uso:** microservicios de alto rendimiento, comunicación backend–backend.
- **Por qué elegirlo:** ofrece baja latencia, tipado fuerte y gran eficiencia en arquitecturas de microservicios.

---

### 6. Socket.IO

Una abstracción sobre WebSockets con fallback automático a otros protocolos.

- **Casos de uso:** chats, notificaciones, sistemas colaborativos que requieren alta confiabilidad.
- **Por qué elegirlo:** maneja reconexiones, fallback, broadcast avanzado y rooms, reduciendo la complejidad de implementación.
...
Link del video: https://youtu.be/cuSU2T3592E
